...
#include "kernel.h"
#include "ebpf_short_flows_completion_exp4.h"
...
static int create_new_flow_infos(struct bpf_elf_map *dt_map, struct bpf_elf_map *c_map, struct flow_tuple *flow_id, __u64 cur_time, struct bpf_sock_ops *skops) {
	...
	new_flow.exp4_last_probability.mantissa = LARGEST_BIT;
	new_flow.exp4_last_probability.exponent = BIAS;
	new_flow.exp4_last_expectation_stability.mantissa = LARGEST_BIT;
	new_flow.exp4_last_expectation_stability.exponent = BIAS;
	new_flow.exp4_last_expectation_change.mantissa = LARGEST_BIT;
	new_flow.exp4_last_expectation_change.exponent = BIAS;
	...
	switch (op) {
		case BPF_SOCK_OPS_PASSIVE_ESTABLISHED_CB: // Call EXP4 for servers (because setting the SRH for request socks does not work)
			...
			if (dst_infos) {
				...
				dst_infos->last_srh_id = flow_info->srh_id;
				...
				bpf_debug("passive established 2\n");
			}
			break;
		...
		case BPF_SOCK_OPS_STATE_CB: // Change in the state of the TCP CONNECTION
			...
			if (skops->args[1] == BPF_TCP_CLOSE) {
				...
				dst_infos = (void *) bpf_map_lookup_elem(&short_dest_map, flow_id.remote_addr);
				if (dst_infos) {
					// Store experience if we use EXP4, otherwise, pure random
					if (USE_EXP3)
						exp4_loss_path(flow_info, dst_infos, skops);
